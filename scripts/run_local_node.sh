...
#
# Examples:
# CHAIN_ID="local-1" HOME_DIR="~/.inco" BLOCK_TIME="1000ms" CLEAN=true sh scripts/run_local_node.sh
...
from_scratch () {
  ...
  # inco1hj5fveer5cjtn4wd6wstzugjfdxzl0xpulae5k
  ...
  # Allocate genesis accounts: 10**27ainco, so 1B INCO
  $BINARY add-genesis-account $KEY 1000000000000000000000000000$DENOM,900test --keyring-backend $KEYRING
  $BINARY add-genesis-account $KEY2 1000000000000000000000000000$DENOM,800test --keyring-backend $KEYRING

  # Sign genesis transaction
  $BINARY gentx $KEY 1000000$DENOM --keyring-backend $KEYRING --chain-id $CHAIN_ID
...
$BINARY start --pruning=nothing  --minimum-gas-prices=0$DENOM --rpc.laddr="tcp://0.0.0.0:$RPC" --json-rpc.address="0.0.0.0:8545" --json-rpc.ws-address="0.0.0.0:8546"